...
- Input validation (e.g. empty checks, format validation) belongs in routes, not in business-level services
### Services
- Do NOT create interfaces for business-level services. Use concrete classes directly.
- Interfaces are only for repositories and 3rd party integrations.
- 3rd party integrations go in their own sub-package under `services/` (e.g. `services.claude` for Claude API). Each sub-package contains the interface and its implementation.
- Business-level services (e.g. `HealthService`, `NotificationService`) stay in the root `services` package.
### Database
...
- The project must compile (`mvn compile`) after each feature is implemented
- All tests must pass (`mvn test`) after each feature is implemented
- Do not move to the next feature until compile + tests pass for the current one
